...
class GameController(GameControllerInterface):
    ...
    def startGame(self):
        ...
        if(self.paused != True):
            ...
            if(self.gameOver != True):
                self.view.drawGame([self.fruit, self.snake], self.speedGame, self.score, self.contFruit)
    ...
    def setSpeeds(self, event):
        ...
        if(not self.key):
            ...
            if((event.key == pygame.K_UP or event.key == pygame.K_w) and self.speedY != self.sizeSprite):
                ...
                self.speedX = 0
                self.key = True
            if((event.key == pygame.K_LEFT or event.key == pygame.K_a) and self.speedX != self.sizeSprite):
                self.speedX = -1*self.sizeSprite
                self.speedY = 0
                self.key = True
            if((event.key == pygame.K_RIGHT or event.key == pygame.K_d) and self.speedX != -1*self.sizeSprite):
    ...
    def itsGameOver(self):
        ...
        if(self.head.rect.x < SPRITE_SIZE or self.head.rect.x > BOARD_MAX_COL * SPRITE_SIZE or self.head.rect.y < BOARD_MIN_ROW * SPRITE_SIZE or self.head.rect.y > BOARD_MAX_ROW * SPRITE_SIZE):
            ...
            for part in self.parts:
                if(self.head.rect.x == part.rect.x and self.head.rect.y == part.rect.y and part != self.head):
                    return True
